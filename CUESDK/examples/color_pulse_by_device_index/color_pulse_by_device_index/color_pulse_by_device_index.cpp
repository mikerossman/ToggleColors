// color_pulse_by_device_index.cpp : Defines the entry point for the console application.
//
...
AvailableKeys getAvailableKeys()
{
	...
	for (auto deviceIndex = 0; deviceIndex < CorsairGetDeviceCount(); deviceIndex++) {
		if (auto deviceInfo = CorsairGetDeviceInfo(deviceIndex)) {
			...
			case CDT_Headset: {
				LedColorsVector keys;
				keys.push_back(CorsairLedColor{ CLH_LeftLogo, 0, 0, 0 });
				keys.push_back(CorsairLedColor{ CLH_RightLogo, 0, 0, 0 });
				availableKeys[deviceIndex] = keys;
			} break;
